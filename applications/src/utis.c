...
void HexStrToByte(const char* source, uint8_t* dest, int sourceLen)
{
    ...
    {
        highByte = my_toupper_ascii(source[i]);
        lowByte  = my_toupper_ascii(source[i + 1]);
...
int8_t parse_utc_time(const char* time_str, calendar_type* rtc_time) {
    ...
    // 检查字符串格式
    if (strlen(time_str) != 17) {
        return -1;
    }
    
    // 解析日期和时间
    if (sscanf(time_str, "%2hu/%2hu/%2hu,%2hu:%2hu:%2hu", 
    ...
    // 转换为完整年份（假设为2000年之后的年份）
    rtc_time->year = 2000 + year;
    rtc_time->month = month;
    rtc_time->date = day;
    
    // 应用时区校正（UTC+8）
    rtc_time->hour = hour + TIME_ZONE_OFFSET;
    rtc_time->min = minute;
    rtc_time->sec = second;
    ...
    if (rtc_time->hour >= 24) {
        ...
        // 检查月份天数
        if (rtc_time->date > get_month_days(rtc_time->year, rtc_time->month)) {
            rtc_time->date = 1;
            rtc_time->month++;
    ...
    // 计算星期
    rtc_time->week = rtc_week_get(rtc_time->year, rtc_time->month, rtc_time->date);
...
const uint16_t Gb698FcsTab[256]={
...
uint8_t calculate_simple_xor(const uint8_t *data, uint32_t data_len) {
    ...
    uint8_t result = data[0]; // 从第一个字节开始
    
    // 从第二个字节开始逐个异或
    for (uint32_t i = 1; i < data_len; i++) {
        result ^= data[i]; // 当前结果与下一个数据字节异或
    }
